 ...
 */
class GoTermRepository extends EntityRepository
{
     public function getEntityFromId($entityId)
    {
        $message="Inside getEntityFromId at GoTermRepository";
        $query = $this->_em->createQuery("
            SELECT a
            FROM EtoxMicromeEntityBundle:GoTerm a
            WHERE a.id= :entityId
        ");
        $query->setParameter('entityId', $entityId);
        $goTerm=$query->getResult();
        if(count($goTerm)==0){
    ...
    {
        $message="Inside getEntityFromName at GoTermRepository";
        $query = $this->_em->createQuery("
            SELECT a
            FROM EtoxMicromeEntityBundle:GoTerm a
