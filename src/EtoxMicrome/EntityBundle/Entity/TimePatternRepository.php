...
/**
 * Time_patternRepository
 *
 * This class was generated by the Doctrine ORM. Add your own custom
 * repository methods below.
 */
class Time_patternRepository extends EntityRepository
{
    public function getEntityFromId($entityId)
    {
        $message="Inside getEntityFromId at TimePatternRepository";
        $query = $this->_em->createQuery("
            SELECT a
            FROM EtoxMicromeEntityBundle:TimePattern a
            WHERE a.id= :entityId
        ");
        $query->setParameter('entityId', $entityId);
        $timePattern=$query->getResult();
        if(count($timePattern)==0){
            $errorMessage="There is no entity with that name ('$entityName')";
            ldd($errorMessage);
        }
        if(count($timePattern)!=1){
        ...
        $entity=$timePattern[0];
        return $entity;
    }

    public function getEntityFromName($entityName)
    {
        $message="Inside getEntityFromName at TimePatternRepository";
        $query = $this->_em->createQuery("
            SELECT a
            FROM EtoxMicromeEntityBundle:TimePattern a
